...
echo(
echo  ============================================================
echo   Ultimate-Selfie-Gen  --  Oldcam V11
echo  ============================================================
...
if not defined TASK_MODEL_PATH (
  echo   [%LAUNCH_TS%] ERROR: face_landmarker.task not found. Oldcam V11 cannot run.
...
set "STAMP_KEY=%STAMP_KEY::=-%"
set "STAMP=%STATE_DIR%\oldcam_v11_%STAMP_KEY:~0,60%.ok"
...
if "%NEED_PIP%"=="0" (
  ...
  if errorlevel 1 (
    echo   [%LAUNCH_TS%] ERROR: MediaPipe Tasks FaceLandmarker API unavailable. Oldcam V11 cannot run.
    echo   Close Python/GUI processes, delete/rebuild venv, and retry.
    set "HAD_ERRORS=1"
    goto DONE
  )
  for %%F in ("%STATE_DIR%\oldcam_v11_*.ok") do del "%%F" >nul 2>&1
