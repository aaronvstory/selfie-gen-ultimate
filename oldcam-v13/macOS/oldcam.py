...
def build_default_output_path(input_path):
    ...
    return str(path.with_name(f"{path.stem}-oldcam-v13{path.suffix}"))


def build_preview_output_path(input_path):
    path = Path(input_path)
    return str(path.with_name(f"{path.stem}-preview-v13{path.suffix}"))
...
def process_frame(image, lut, vignette_mask, args, rng=None, state=None):
    ...
    # Pure physical camera hardware emulation only — flagship phone in bright sun.
