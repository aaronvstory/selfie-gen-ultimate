...
| Output resolution | Use `automation.rppg.resolve_produced_output()` — the single source of truth. It accepts the exact `--output` path **or** the newest `{stem}-rppg*{ext}` sibling. The GUI queue + the harness both call it. |
...
| Layer | File | What |
|-------|------|------|
| Manifest step registry | `automation/manifest.py` `STEP_NAMES` | `"rppg"` after `"oldcam"` (else `update_step` raises `Unknown step: rppg`) |
...
| Automation module | `automation/rppg.py` | `run_rppg`, `build_rppg_output_path`, `resolve_produced_output`, `resolve_rppg_launcher` (mirrors `automation/oldcam.py`) |
