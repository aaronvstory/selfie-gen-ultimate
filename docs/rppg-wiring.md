...
| Config defaults | `automation/config.py` `AUTOMATION_DEFAULTS` | `automation_rppg_enabled` (False), `automation_rppg_mode` ("iterative" — PR #43, was "inject"), `automation_rppg_iterate_from_baseline` (True), `automation_rppg_skip_diagnosis` (True), `automation_rppg_skip_kinematic_gate` (True), `automation_rppg_landmark_stride` (1 — quality-first default reverting the 2026-05-25 v2.5 speedup pass: PR `feat/v2.6-auto-gpu-polish` 2026-05-27 dialed back to every-frame detection because Kling output occasionally moves faster than the prompt asks for and silently degrading those clips is worse than the slowdown on the slow-majority. GUI alias `rppg_landmark_stride`; raise to 3 for explicit 3-5x speedup on confirmed near-still source — and once the auto-NVIDIA bootstrap activates CuPy on a CUDA host, the per-frame mediapipe cost matters far less anyway.), `automation_rppg_required` (False), `automation_rppg_metrics_in_filename` (False). `automation_recommended_defaults_version` bumped from 1 to 2 when iterative became default. |
