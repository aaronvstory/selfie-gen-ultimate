...
echo(
echo  ============================================================
echo   Ultimate-Selfie-Gen  --  Oldcam V14 (Forensic Daylight)
echo  ============================================================
...
set "STAMP_KEY=%STAMP_KEY::=-%"
set "STAMP=%STATE_DIR%\oldcam_v14_%STAMP_KEY:~0,60%.ok"
...
if "%NEED_PIP%"=="0" (
  ...
  echo   [%LAUNCH_TS%] Syncing Oldcam V14 dependencies...
  "%PYTHON_CMD%" -m pip install -r "%SCRIPT_DIR%requirements.txt" >nul 2>&1
  if errorlevel 1 (
    echo   [%LAUNCH_TS%] ERROR: Failed to install Oldcam V14 dependencies.
    echo   Close running Python/GUI processes and retry.
    set "HAD_ERRORS=1"
    goto DONE
  )
  for %%F in ("%STATE_DIR%\oldcam_v14_*.ok") do del "%%F" >nul 2>&1
