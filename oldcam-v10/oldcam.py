...
def build_default_output_path(input_path):
    ...
    return str(path.with_name(f"{path.stem}-oldcam-v10{path.suffix}"))


def build_preview_output_path(input_path):
    path = Path(input_path)
    return str(path.with_name(f"{path.stem}-preview-v10{path.suffix}"))
...
def synchronize_base_frequency(
    ...
    Returns:
        Dominant temporal frequency in Hz (clamped to [0.8, 1.8] range —
        narrowed from [0.7, 4.0] to keep FFT locked on heartbeat-band
        frequencies. Real heartbeat is 0.8-1.3 Hz (48-78 BPM); the wider
        old range let the FFT lock onto AI processing artifacts and strobe
        the face at 240 "beats" per minute, the V10 "siren" effect).
    """
