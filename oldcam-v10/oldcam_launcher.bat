...
echo(
echo  ============================================================
echo   Ultimate-Selfie-Gen  --  Oldcam V10
echo  ============================================================
...
if not defined TASK_MODEL_PATH (
  echo   [%LAUNCH_TS%] ERROR: face_landmarker.task not found. Oldcam V10 cannot run.
...
set "STAMP_KEY=%STAMP_KEY::=-%"
set "STAMP=%STATE_DIR%\oldcam_v10_%STAMP_KEY:~0,60%.ok"
...
if "%NEED_PIP%"=="0" (
  ...
  echo   [%LAUNCH_TS%] Syncing Oldcam V10 dependencies...
  set "REQ_FILTERED=%STATE_DIR%\oldcam_v10_req_filtered.txt"
  ...
  if errorlevel 1 (
    ...
    echo   [%LAUNCH_TS%] ERROR: Failed to install Oldcam V10 dependencies.
  ...
  if errorlevel 1 (
    for %%F in ("%REQ_FILTERED%") do del "%%F" >nul 2>&1
    echo   [%LAUNCH_TS%] ERROR: Failed to install MediaPipe for Oldcam V10.
    echo   Close running Python/GUI processes and retry.
    set "HAD_ERRORS=1"
    goto DONE
  )
  ...
  if errorlevel 1 (
    echo   [%LAUNCH_TS%] ERROR: MediaPipe Tasks FaceLandmarker API unavailable. Oldcam V10 cannot run.
    echo   Close Python/GUI processes, delete/rebuild venv, and retry.
    set "HAD_ERRORS=1"
    goto DONE
  )
  for %%F in ("%STATE_DIR%\oldcam_v10_*.ok") do del "%%F" >nul 2>&1
