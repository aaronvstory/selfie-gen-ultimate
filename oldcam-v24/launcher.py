#!/usr/bin/env python3
"""Cross-platform launcher for Oldcam V24 inside the Creative Suite."""
...
# oldcam-v24/, so only that dir is on sys.path[0]. Without this bootstrap
...
def choose_files() -> list[str]:
    ...
    return select_open_files(
        title="Select media files for Oldcam V24",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Launch Oldcam V24 with a file picker when no input files are provided."
    )
