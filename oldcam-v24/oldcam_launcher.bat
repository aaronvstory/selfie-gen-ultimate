...
echo(
echo  ============================================================
echo   Ultimate-Selfie-Gen  --  Oldcam V24 (Crush Laundromat)
echo  ============================================================
...
if errorlevel 1 (
  ...
  if not "%SELFIEGEN_PYTHON%"=="" (
    echo   [%LAUNCH_TS%] ERROR: SELFIEGEN_PYTHON points at Python !PY_ACTUAL!, but Oldcam v24 requires 3.9-3.12. Unset it or point at python3.11.
...
rem --- V24 needs no MediaPipe / face_landmarker.task: resolution-roundtrip pipeline.
...
set "STAMP_KEY=%STAMP_KEY::=-%"
set "STAMP=%STATE_DIR%\oldcam_v24_%STAMP_KEY:~0,60%.ok"
...
if "%NEED_PIP%"=="0" (
  ...
  echo   [%LAUNCH_TS%] Syncing Oldcam V24 dependencies...
  "%PYTHON_CMD%" -m pip install !CC! -r "%SCRIPT_DIR%requirements.txt" >nul 2>&1
  if errorlevel 1 (
    echo   [%LAUNCH_TS%] ERROR: Failed to install Oldcam V24 dependencies.
    echo   Close running Python/GUI processes and retry.
    set "HAD_ERRORS=1"
    goto DONE
  )
  for %%F in ("%STATE_DIR%\oldcam_v24_*.ok") do del "%%F" >nul 2>&1
