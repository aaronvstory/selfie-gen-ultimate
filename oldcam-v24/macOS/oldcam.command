...
find_repo_root() {
  ...
  while [ -n "$cur" ] && [ "$cur" != "/" ]; do
    if [ -f "$cur/kling_automation_ui.py" ] && [ -f "$cur/requirements.txt" ] && [ -d "$cur/oldcam-v24" ]; then
      printf '%s\n' "$cur"
      return 0
    fi
    cur="$(dirname "$cur")"
...
if ! "$PYTHON_CMD" -c 'import sys; raise SystemExit(0 if (3, 9) <= sys.version_info[:2] < (3, 13) else 2)' >/dev/null 2>&1; then
  ...
  if [ -n "${SELFIEGEN_PYTHON:-}" ] || [ -n "${SELFIEGEN_VENV_DIR:-}" ]; then
    echo "[ERROR] Your SELFIEGEN_PYTHON / SELFIEGEN_VENV_DIR override points at Python ${PY_ACTUAL}, but Oldcam v24 requires 3.9-3.12. Unset the override or point it at a supported interpreter."
  else
...
echo "[oldcam-v24] Using $ENV_KIND"
echo "[oldcam-v24] Python: $PYTHON_CMD"

# V24 is the resolution-roundtrip profile — no MediaPipe / face_landmarker.task required.
...
if [ ! -f "$STAMP" ] || ! "$PYTHON_CMD" -c "import cv2, numpy" >/dev/null 2>&1; then
  ...
  rm -f "$STATE_DIR"/oldcam_v24_*.ok
  echo ok > "$STAMP"
fi
pick_files() {
  osascript <<'APPLESCRIPT'
set selectedFiles to choose file with prompt "Select one or more media files for Oldcam V24" with multiple selections allowed
set outputText to ""
