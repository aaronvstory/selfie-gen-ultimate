...
if [ ! -f "$STAMP" ] || ! "$PYTHON_CMD" -c "import cv2, numpy" >/dev/null 2>&1 || ! "$PYTHON_CMD" -c "$MP_VALIDATE_CMD" >/dev/null 2>&1; then
  ...
  if ! "$PYTHON_CMD" -c "$MP_VALIDATE_CMD" >/dev/null 2>&1; then
    echo "MediaPipe installed but Tasks FaceLandmarker API unavailable. Oldcam v9 cannot run."
...
set selectedFiles to choose file with prompt "Select one or more media files for Oldcam V9" with multiple selections allowed
set outputText to ""
