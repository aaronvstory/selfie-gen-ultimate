...
# V25: width of the uniform rolling temporal average applied to EVERY
# output frame (the temporal analog of V24's spatial low-pass). Each
# written frame = mean of the last TEMPORAL_WINDOW processed frames.
# 5 spans ~0.2s @ 24fps — wide enough to smear a burst/freeze cadence
# into a continuous flow, narrow enough to keep gross motion legible.
# 1 disables it (V25 == V24). Tune up if a clip's cadence is still
# detectable, down if motion turns to mush.
TEMPORAL_WINDOW = 5
...
def build_default_output_path(input_path):
    # -oldcam-v25 so resemble-score classifies it as "Oldcam v25".
    path = Path(input_path)
    return str(path.with_name(f"{path.stem}-oldcam-v25{path.suffix}"))


def build_preview_output_path(input_path):
    path = Path(input_path)
    return str(path.with_name(f"{path.stem}-preview-v25{path.suffix}"))
...
def naturalize_video(input_path: str, output_path: str, args: argparse.Namespace) -> None:
    ...
    # V25: rolling window of the last TEMPORAL_WINDOW processed (post-
    # ghosting) frames. Each written frame is their mean — a uniform
    # temporal low-pass that smears an irregular burst/freeze cadence
    # into a continuous flow. Window of 1 -> identical to V24.
    _tw = max(1, int(TEMPORAL_WINDOW))
    _temporal_buf: list = []
    ...
    try:
        while True:
            ...
            # V25: uniform temporal resmooth — mean of the trailing
            # TEMPORAL_WINDOW processed frames. Done in float then cast
            # back so it cannot wrap uint8 / bias luma. Applied to the
            # processed (post-ghosting) frame and BEFORE the preview
            # composite so the preview reflects the real output.
            if _tw > 1:
