#!/usr/bin/env python3
"""
oldcam_v19.py - V19 "Uniform Grain Dither" Virtual Hardware Simulator
                (STANDALONE TEST)

>>> STANDALONE EXPERIMENT — NOT wired into the app. <<<
This file lives in oldcam-testing/ and is intentionally NOT registered in
queue_manager._discover_oldcam_versions, the launchers, config_panel, or the
main test suite. It exists only to A/B a V19 idea against V15 through the
resemble-score deepfake API before deciding whether to promote it. Run it
directly:  python oldcam-testing/oldcam_v19.py <video> [--ghosting 0.18 ...]

V19 = clean V15 + a UNIFORM, STATIC-AMPLITUDE GRAIN DITHER on EVERY frame.
NO motion gating of any kind.

The synthesis from V16/V17/V18 (4 API calls): every *localized,
motion-gated* per-frame manipulation lost — warp AND blur, gentle AND
hard — because each creates a frame whose frequency profile deviates
*locally*, the exact anomaly the Resemble detector spots. V15 wins
because it treats every frame identically (uniform mp4v→CRF23 crush):
there is no localized signature.

V19 is the first GLOBAL/UNIFORM idea. Physics: a real CMOS sensor never
shoots a perfectly clean daylight frame — there is always a tiny, uniform
photon-shot dither across the whole image, the same on every frame. V14
tried a sensor floor but put it on a LOSSLESS FFV1 encode, so the API
read the noise math perfectly and flagged it. V19 adds a tiny static
dither and lets the V15 "Laundromat" (mp4v → H.264 CRF 23) crush it into
the macro-blocks, breaking up Kling's residual diffusion flicker
UNIFORMLY across the whole clip with NO localized motion anomaly.

Critically different from V14: V14's floor was sub-perceptual and
preserved losslessly (a clean periodic tell). V19's dither is a coarse
uniform integer perturbation that exists ONLY to be destroyed by the
lossy chain — it never survives to the final file as readable math.

Inherited from V15 verbatim:

  - V14's corrected per-frame math: true multiplicative AWB color-temperature
    drift, smoothstep highlight bloom, stream-copied original audio, np.rint
    casts (no darkening bias), cached vignette mask.
  - V13's noise-free philosophy re: the V14 *sensor floor*:
    `apply_daylight_sensor_floor` stays deleted (it was the lossless tell).
  - V12's temporal blending: --ghosting (default 0.18).
  - "Laundromat" lossy chain: mp4v temp + H.264 --crf 23 (clamp 10-28).
  - Baseline (un-coupled) OIS jitter + rolling shutter, exactly as V15.

V19 additions:

  - apply_uniform_grain_dither(): a single low-amplitude additive integer
    dither applied IDENTICALLY to every frame (no motion_multiplier, no
    gating). Placed LATE in process_frame (after all optics) so it rides
    the finished image like a real sensor's photon-shot floor — then the
    mp4v Laundromat crushes it into macro-blocks. Amplitude is a module
    constant (GRAIN_DITHER_AMPLITUDE) so the next tweak is one line.
...
# V19 uniform grain dither: the ONLY knob that differs from V15. Peak signed
# amplitude (in 8-bit levels) of the per-pixel integer dither added to EVERY
# frame identically. 4 is a deliberate, clearly-supra-sensor value: it must be
# coarse enough to perturb Kling's residual diffusion flicker yet small enough
# that the mp4v→CRF23 Laundromat crushes it into macro-blocks (so it never
# survives as readable noise math, V14's mistake). One-line tune + new vN.
GRAIN_DITHER_AMPLITUDE = 4  # +/- levels, uniform, same on every frame
...
def build_default_output_path(input_path):
    # -oldcam-v19 suffix so resemble-score classifies it as "Oldcam v19"
    # (it keys on the *-oldcam-vN.<ext> filename pattern).
    path = Path(input_path)
    return str(path.with_name(f"{path.stem}-oldcam-v19{path.suffix}"))


def build_preview_output_path(input_path):
    path = Path(input_path)
    return str(path.with_name(f"{path.stem}-preview-v19{path.suffix}"))
...
def apply_soft_ois_jitter(image, state, rng):
    # V18: baseline V15 OIS jitter — NO motion coupling (V16/V17 proved the
    # geometric coupling adds detectable warp). V18 leaves the geometry alone.
    h, w = image.shape[:2]
...
def apply_soft_rolling_shutter(image, state, rng):
    # V18: baseline V15 rolling shutter — NO motion coupling (V16/V17 proved
    # the motion-coupled shear adds detectable tearing). Geometry untouched.
    h, w = image.shape[:2]
...
def apply_uniform_grain_dither(image: np.ndarray, rng) -> np.ndarray:
    """V19: a tiny UNIFORM integer dither added identically to every frame.

    NOT motion-gated and NOT spatially structured — a fresh per-pixel
    signed perturbation in [-A, +A] across the whole frame, every frame,
    with the same statistics. It models a real CMOS photon-shot floor and
    exists only to be crushed by the mp4v→CRF23 Laundromat into the H.264
    macro-blocks (it must NOT survive as readable noise math — that was
    V14's lossless mistake). Because the statistics are identical on every
    frame there is no localized signature for the detector to flag (the
    losing pattern from V16/V17/V18).
    """
    a = int(GRAIN_DITHER_AMPLITUDE)
    if a <= 0:
        return image
    # int16 add then clip — avoids uint8 wrap and the slow darkening bias
    # a truncating cast would introduce over a long clip.
    noise = rng.integers(-a, a + 1, size=image.shape, dtype=np.int16)
...
def process_frame(
    ...
    # V19: Uniform Grain Dither Profile = clean V15 pipeline + ONE global,
    # static-amplitude dither on EVERY frame (no motion gating — V16/V17/V18
    # proved localized per-frame manipulation is the losing pattern). V14
    # math/encoding + V13 noise-free re: the V14 sensor floor. No face
    # tracking, no AE hunting, no rPPG. Temporal blending in naturalize_video.

    # 1. Physical motion — BASELINE V15 (no motion coupling).
    ...
    # 5. V19 NEW: uniform grain dither — applied LAST, identically on every
    #    frame, so it rides the finished image like a real sensor's
    #    photon-shot floor. The mp4v→CRF23 Laundromat then crushes it into
    #    macro-blocks (it must NOT survive as readable noise math — that was
    #    V14's lossless tell). NO motion gating: same statistics every frame.
    image = apply_uniform_grain_dither(image, rng)
...
def naturalize_video(input_path: str, output_path: str, args: argparse.Namespace) -> None:
    ...
    # V19: NO motion detection — the dither is uniform on every frame, so
    # there is nothing to gate. (V16/V17/V18's motion_multiplier block is
    # gone entirely; localized per-frame manipulation was the losing
    # pattern that this version deliberately abandons.)
    ...
    try:
        while True:
            ...
            current_processed = process_frame(
                frame, None, vignette_mask, args, rng, state
            )
