#!/usr/bin/env python3
"""
oldcam_v16.py - V16 "Dynamic Stress" Virtual Hardware Simulator (STANDALONE TEST)
...
main test suite. It exists only to A/B a V16 idea against V15 through the
resemble-score deepfake API before deciding whether to promote it. Run it
directly:  python oldcam-testing/oldcam_v16.py <video> [--ghosting 0.18 ...]

V16 keeps EVERYTHING from V15 (the math, the lossy mp4v -> CRF 23 encode, the
0.18 ghosting, zero synthetic noise) and adds ONE idea: the optical physics
now REACT to motion.

V15's "Laundromat" double-lossy compression scored ~0.42 on the Resemble API
(out of the "fake" band) by crushing the latent-diffusion noise. But the
detector still spikes during rapid head turns — AI struggles with structural
consistency across fast perspective shifts. V16 measures frame-to-frame pixel
velocity and DYNAMICALLY spikes rolling-shutter shear and OIS jitter only
during high-motion moments, physically blurring the AI's worst structural
frames right before the compressor crushes them. When the subject is still,
V16 behaves exactly like V15 (motion_multiplier == 0.0 → no extra stress).
...
V16 additions:
  ...
  - Motion-coupled rolling shutter: shear * (1.0 + motion_multiplier * 5.0).
  - Motion-coupled OIS jitter:   offset * (1.0 + motion_multiplier * 8.0).
...
def build_default_output_path(input_path):
    # -oldcam-v16 suffix so resemble-score classifies it as "Oldcam v16"
    # (it keys on the *-oldcam-vN.<ext> filename pattern).
    path = Path(input_path)
    return str(path.with_name(f"{path.stem}-oldcam-v16{path.suffix}"))


def build_preview_output_path(input_path):
    path = Path(input_path)
    return str(path.with_name(f"{path.stem}-preview-v16{path.suffix}"))
...
def apply_soft_ois_jitter(image, state, rng, motion_multiplier: float = 0.0):
    ...
    # V16: drive the OIS mechanism to its physical boundary during fast
    # motion. At motion_multiplier == 0 (subject still) this is exactly V15;
    # at 1.0 the translation offset is amplified up to 9x, inducing the
    # heavy motion blur that hides the AI's worst structural frames before
    # the mp4v compressor crushes them.
    boost = 1.0 + (float(motion_multiplier) * 8.0)
...
def apply_soft_rolling_shutter(image, state, rng, motion_multiplier: float = 0.0):
    ...
    # V16: a real CMOS sensor's rolling-shutter skew scales with how fast the
    # scene crosses the frame. Spike the shear up to 6x during a fast head
    # turn (motion_multiplier -> 1.0); at 0.0 this is identical to V15.
    # The state feedback below uses the *base* shear (pre-boost) so the
    # mean-reverting velocity model stays stable across motion bursts.
    shear_val = float(np.clip(shear_val, -0.0018, 0.0018))
    state["rs_velocity"] = rs_velocity - shear_val * 0.08
    boosted_shear = shear_val * (1.0 + (float(motion_multiplier) * 5.0))
...
def process_frame(
    ...
    # V16: Dynamic Stress Profile = V15 (Temporal Mute) + motion-coupled
    # optics. V14 math/encoding + V13 noise-free philosophy. No sensor floor,
    # no face tracking, no AE hunting, no rPPG. Temporal blending is applied
    # in naturalize_video. The ONLY V16 change vs V15 is that the two physical
    # motion passes scale with motion_multiplier (0.0 == V15 behaviour).

    # 1. Physical motion (motion-coupled in V16).
