#!/usr/bin/env python3
"""
oldcam_v24.py - V24 "Resolution Round-Trip ×0.40 + Sharpen" Virtual
                Hardware Simulator (STANDALONE TEST)
...
the main test suite. It exists only to A/B a V24 idea against the merged
production V15 through the resemble-score deepfake API. Run it directly:
  python oldcam-testing/oldcam_v24.py <video> [--ghosting 0.18 ...]

V24 attacks the "smudge" problem of the resolution round-trip while
keeping a great score. V21 (×0.5) → frame_mean 0.0249; V23 (×0.35) →
0.0094 (17× better than V15) BUT visually quite smudged: a single big
downscale + a *bilinear* upscale is soft everywhere.

V24 = V21's round-trip with THREE anti-smudge changes vs V23:
  1. RESOLUTION_SCALE 0.35 → 0.40 — back off the most aggressive
     destruction slightly (less inherent smudge to fight; still far past
     V21's 0.5 which already scored 0.025).
  2. Upscale interpolation INTER_LINEAR → INTER_LANCZOS4 — reconstructs
     edges far more crisply from the SAME destroyed low-res frame
     (bilinear is the blurriest common kernel).
  3. A light unsharp-mask AFTER the upscale (UNSHARP_AMOUNT) — restores
     *perceived* edge crispness. Crucially this does NOT recreate the
     AI's destroyed high-frequency fingerprint: that signature was
     annihilated at the small resolution and cannot be invented back;
     sharpening only amplifies what survived. Real phone ISPs sharpen
     aggressively post-readout, so this is also physically faithful.

Still essentially destructive: the AI fingerprint is killed by the
downscale before anything is sharpened. The sharpen amplifies surviving
(real-image) structure, not synthetic signal — distinct from the
V14/V19 additive failures. RESOLUTION_SCALE = 1.0 → V24 == V15 exactly.

V21/V23 origin: a byte-for-byte copy of the MERGED production V15
(origin/main:oldcam-v15/oldcam.py) with the round-trip as the only
process_frame change. The conclusive V16–V19 synthesis: V15 wins because
it is purely DESTRUCTIVE; additive signals always lose. V20–V23 confirmed
"destroy more, uniformly" monotonically lowers the score — the binding
constraint is now VISUAL QUALITY, which is exactly what V24 targets.
