#!/usr/bin/env python3
"""
oldcam_v23.py - V23 "Resolution Round-Trip ×0.35" Virtual Hardware
                Simulator (STANDALONE TEST)
...
the main test suite. It exists only to A/B a V23 idea against the merged
production V15 through the resemble-score deepfake API. Run it directly:
  python oldcam-testing/oldcam_v23.py <video> [--ghosting 0.18 ...]

V23 == V21 "Resolution Round-Trip" with a SINGLE constant changed:
RESOLUTION_SCALE 0.5 -> 0.35 (the hardest spatial low-pass in this
floor-finding sweep: V21=0.5 → 0.0249, V22=0.4, V23=0.35). Probes where
crushing resolution further stops helping and starts reading as "low
quality". Pure destructive; no added signal; apply_resolution_roundtrip
+ process_frame are byte-identical to V21.

V21 is a byte-for-byte copy of the MERGED production V15 (cloned from
origin/main:oldcam-v15/oldcam.py) with EXACTLY ONE change: each frame is
downscaled then upscaled back (a uniform spatial low-pass) before the V15
encode. Purely DESTRUCTIVE — adds NOTHING.

The conclusive synthesis from V16–V19 (5 API calls): V15 wins because it
is purely DESTRUCTIVE — it only *removes* information. Every additive
signal (warp/blur/grain, V14's floor) lost. The only axis that helps is
*removing more*, uniformly. V20 tests "compress harder" (a 2nd Laundromat
pass); V21 tests a DIFFERENT destructive axis: a spatial-resolution
low-pass. Downscale (INTER_AREA — proper anti-aliased decimation) then
upscale back (INTER_LINEAR) annihilates the high-frequency band where
Kling's diffusion fingerprint lives, uniformly on every frame, without
adding any signal. Then the V15 mp4v→CRF23 Laundromat crushes what's
left. RESOLUTION_SCALE is a module constant (0.5 = halve then restore).
At RESOLUTION_SCALE = 1.0 V21 == V15 exactly.
...
# V23: the ONLY change vs V21 (which used 0.5). Each frame is downscaled by
# this factor (INTER_AREA, proper anti-aliased decimation) then upscaled
# back (INTER_LINEAR) — a uniform spatial low-pass that annihilates the
# high-frequency band where Kling's diffusion fingerprint lives, adding NO
# signal. 0.35 = hardest crush in the V21/V22/V23 sweep; floor probe.
...
def build_default_output_path(input_path):
    # -oldcam-v23 so resemble-score classifies it as "Oldcam v23".
    path = Path(input_path)
    return str(path.with_name(f"{path.stem}-oldcam-v23{path.suffix}"))


def build_preview_output_path(input_path):
    path = Path(input_path)
    return str(path.with_name(f"{path.stem}-preview-v23{path.suffix}"))
...
def apply_resolution_roundtrip(image: np.ndarray) -> np.ndarray:
    """V21: downscale then upscale back — a uniform spatial low-pass.

    INTER_AREA for the downscale (proper anti-aliased decimation — the
    correct filter for shrinking) then INTER_LINEAR back to the original
    size. This destroys the high-frequency band where Kling's diffusion
    fingerprint lives, identically on every frame, and adds NO signal. The
    output is exactly the original WxH so the rest of the V15 pipeline /
    the writer are unaffected. RESOLUTION_SCALE == 1.0 is a no-op (==V15).
    """
