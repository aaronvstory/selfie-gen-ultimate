#!/usr/bin/env python3
"""
oldcam_v20.py - V20 "Double Laundromat" Virtual Hardware Simulator
                (STANDALONE TEST)
...
the main test suite. It exists only to A/B a V20 idea against the merged
production V15 through the resemble-score deepfake API. Run it directly:
  python oldcam-testing/oldcam_v20.py <video> [--ghosting 0.18 ...]

V20 is a byte-for-byte copy of the MERGED production V15 (cloned from
origin/main:oldcam-v15/oldcam.py) with EXACTLY ONE change: after the V15
pipeline writes its H.264 file, run the whole mp4v→H.264 Laundromat a
SECOND time on it. Purely DESTRUCTIVE — adds NOTHING.

The conclusive synthesis from V16–V19 (5 API calls): V15 wins because it
is purely DESTRUCTIVE — it only *removes* information (the mp4v→CRF23
crush of Kling's diffusion artifacts). Every version that ADDED a
synthetic signal lost — warp, blur, grain — uniform or localized,
lossy-crushed or not (an added statistic is learnable; V14/V19 proved
this). The only axis that has ever helped is *removing more*, uniformly.

V20 tests the most direct extension: if one lossy generation helps, does
a second? It re-decodes the V15 output and runs it through mp4v → H.264
CRF 23 AGAIN (4 lossy generations overall). No new signal — a deeper
crush. Risk: it may over-compress into "low-quality blocky" territory the
detector flags the way V18's blur did — but it remains subtractive-only.
The second pass is toggled by the SECOND_PASS module constant; with it
off, V20 == V15 exactly.
...
# V20: the ONLY change vs the merged V15. When True, after the V15 pipeline
# writes its H.264 file, the whole mp4v→H.264 CRF23 Laundromat is run a
# SECOND time on that file (4 lossy generations overall). Purely
# destructive — adds no signal. False -> V20 is byte-identical to V15.
SECOND_PASS = True
...
def build_default_output_path(input_path):
    # -oldcam-v20 so resemble-score classifies it as "Oldcam v20".
    path = Path(input_path)
    return str(path.with_name(f"{path.stem}-oldcam-v20{path.suffix}"))


def build_preview_output_path(input_path):
    path = Path(input_path)
    return str(path.with_name(f"{path.stem}-preview-v20{path.suffix}"))
...
def second_lossy_pass(output_path: str, args: argparse.Namespace) -> None:
    """V20: re-run the whole mp4v→H.264 Laundromat on the finished file.

    Purely destructive — re-decode the V15-pass output, re-encode every
    frame through an mp4v temp (OpenCV) then H.264 CRF 23 (FFmpeg), audio
    stream-copied from the first-pass file. Adds NO signal; just a deeper
    crush. No-op (with a clear message) if anything is unavailable, so a
    failed 2nd pass never destroys the good V15-pass output.
    """
    if not SECOND_PASS:
        return
    src = Path(output_path)
    if not src.is_file():
        print("Second pass skipped: first-pass output missing.")
        return
    cap = cv2.VideoCapture(str(src))
    if not cap.isOpened():
        print("Second pass skipped: cannot reopen first-pass output.")
        return
    fps = cap.get(cv2.CAP_PROP_FPS) or 30.0
    w = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    h = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    tmp2 = str(src.with_name(f"{src.stem}.tmp_pass2.mp4"))
    writer = cv2.VideoWriter(tmp2, cv2.VideoWriter_fourcc(*"mp4v"), fps, (w, h))
    if not writer.isOpened():
        cap.release()
        writer.release()
        print("Second pass skipped: mp4v writer unavailable.")
        return
    try:
        while True:
            ok, frame = cap.read()
            if not ok:
                break
            writer.write(frame)  # decode→re-encode only; NO pixel change
    finally:
        cap.release()
        writer.release()

    if not ffmpeg_available():
        # Keep the (now mp4v-degraded) 2nd-pass temp as the output: still a
        # second lossy generation, just without the H.264 leg / audio.
        try:
            Path(tmp2).replace(src)
            print(f"Second pass (mp4v-only, no FFmpeg): {output_path}")
        except OSError:
            Path(tmp2).unlink(missing_ok=True)
            print("Second pass failed (no FFmpeg, replace error); kept pass 1.")
        return

    crf = str(int(getattr(args, "crf", 23)))
    final2 = str(src.with_name(f"{src.stem}.pass2.mp4"))
    # Audio is stream-copied from the first-pass file (input 1) so the 2nd
    # video pass costs another generation but the audio is untouched.
    command = [
        "ffmpeg", "-y", "-i", tmp2, "-i", str(src),
        "-map", "0:v:0", "-map", "1:a:0?",
    ...
    try:
        # Audited safe: argv list, shell=False — no shell interpretation.
        # nosemgrep: python.lang.security.audit.dangerous-subprocess-use-audit
        subprocess.run(
            command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
            check=True, timeout=120,
        )
        Path(final2).replace(src)
        print(f"Second Laundromat pass complete: {output_path}")
    except (subprocess.CalledProcessError, subprocess.TimeoutExpired):
        Path(final2).unlink(missing_ok=True)
        print("Second pass FFmpeg failed; kept the good first-pass output.")
    finally:
        Path(tmp2).unlink(missing_ok=True)
...
def naturalize_video(input_path: str, output_path: str, args: argparse.Namespace) -> None:
    ...
    # V20: the one change vs V15 — grind the finished file through the
    # mp4v→H.264 Laundromat a second time (purely destructive, no signal).
    second_lossy_pass(output_path, args)
