#!/usr/bin/env python3
"""
oldcam_v21.py - V21 "Resolution Round-Trip" Virtual Hardware Simulator
                (STANDALONE TEST)
...
the main test suite. It exists only to A/B a V21 idea against the merged
production V15 through the resemble-score deepfake API. Run it directly:
  python oldcam-testing/oldcam_v21.py <video> [--ghosting 0.18 ...]
...
# V21: the ONLY change vs the merged V15. Each frame is downscaled by this
# factor (INTER_AREA, proper anti-aliased decimation) then upscaled back
# (INTER_LINEAR) — a uniform spatial low-pass that annihilates the
# high-frequency band where Kling's diffusion fingerprint lives, adding NO
# signal. 0.5 = halve then restore. 1.0 -> V21 is byte-identical to V15.
...
def build_default_output_path(input_path):
    # -oldcam-v21 so resemble-score classifies it as "Oldcam v21".
    path = Path(input_path)
    return str(path.with_name(f"{path.stem}-oldcam-v21{path.suffix}"))


def build_preview_output_path(input_path):
    path = Path(input_path)
    return str(path.with_name(f"{path.stem}-preview-v21{path.suffix}"))
