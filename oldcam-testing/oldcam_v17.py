#!/usr/bin/env python3
"""
oldcam_v17.py - V17 "Dynamic Stress (Gentle)" Virtual Hardware Simulator
                (STANDALONE TEST)
...
main test suite. It exists only to A/B a V17 idea against V15 through the
resemble-score deepfake API before deciding whether to promote it. Run it
directly:  python oldcam-testing/oldcam_v17.py <video> [--ghosting 0.18 ...]

V17 = V16's motion-coupling idea with MUCH GENTLER gains.

V16 ("Dynamic Stress") coupled rolling-shutter shear ×(1+m·5) and OIS jitter
×(1+m·8) to a per-frame motion velocity. Resemble A/B verdict (GISELLE clip):
V16 frame-mean 0.1884 vs V15's 0.1605 — a REGRESSION. The aggressive
geometric warp during the head turn *raised* the peak frame score
(0.42 → 0.54): the distortion added its own detectable artifacts instead of
masking the AI's. V17 keeps the exact same mechanism but dials the gains
way down to test whether V16 simply overshot:

    MOTION_OIS_GAIN = 2.0   (was 8.0 in V16)
    MOTION_RS_GAIN  = 1.5   (was 5.0 in V16)

Everything else is V15 verbatim. At motion_multiplier == 0.0 (subject still)
V17 is identical to V15, so the only variable under test is the *magnitude*
...
V17 additions (= V16's, gentler):

  - Crude per-frame motion velocity (grayscale absdiff -> threshold -> sum,
    normalized to a 0.0-1.0 motion_multiplier on resolution area).
  - Motion-coupled rolling shutter: shear * (1.0 + motion_multiplier * 1.5).
  - Motion-coupled OIS jitter:   offset * (1.0 + motion_multiplier * 2.0).
  - Both gains are module constants (MOTION_OIS_GAIN / MOTION_RS_GAIN) so
    the next experiment is a one-line tweak + a new vN file.
...
# V17 motion-coupling gains (the ONLY knobs that differ from V15). Effective
# scale at full motion is (1 + gain): OIS offset and rolling-shutter shear.
# V16 used 8.0 / 5.0 and regressed (warp too aggressive → raised the head-turn
# peak). V17 dials them down to test whether the idea works at all when gentle.
# Each future experiment = new oldcam_vN.py with different values here.
MOTION_OIS_GAIN = 2.0  # V16 was 8.0
MOTION_RS_GAIN = 1.5   # V16 was 5.0
...
def build_default_output_path(input_path):
    # -oldcam-v17 suffix so resemble-score classifies it as "Oldcam v17"
    # (it keys on the *-oldcam-vN.<ext> filename pattern).
    path = Path(input_path)
    return str(path.with_name(f"{path.stem}-oldcam-v17{path.suffix}"))


def build_preview_output_path(input_path):
    path = Path(input_path)
    return str(path.with_name(f"{path.stem}-preview-v17{path.suffix}"))
...
def apply_soft_ois_jitter(image, state, rng, motion_multiplier: float = 0.0):
    h, w = image.shape[:2]
    ...
    # V17: motion-couple the OIS offset (gentler than V16's ×8). At
    # motion_multiplier == 0 (subject still) this is exactly V15; at 1.0 the
    # offset is amplified by (1 + MOTION_OIS_GAIN). V16's ×8 over-warped and
    # raised the head-turn peak — V17 tests a much milder coupling.
    boost = 1.0 + (float(motion_multiplier) * MOTION_OIS_GAIN)
    transform = np.float32([[1, 0, x * boost], [0, 1, y * boost]])
...
def apply_soft_rolling_shutter(image, state, rng, motion_multiplier: float = 0.0):
    h, w = image.shape[:2]
    ...
    # V17: motion-couple the rolling-shutter skew (gentler than V16's ×5).
    # A real CMOS sensor's shear scales with how fast the scene crosses the
    # frame; at motion_multiplier 1.0 the shear is amplified by
    # (1 + MOTION_RS_GAIN), at 0.0 it is identical to V15. The state feedback
    # below uses the *base* shear (pre-boost) so the mean-reverting velocity
    # model stays stable across motion bursts.
    shear_val = float(np.clip(shear_val, -0.0018, 0.0018))
    state["rs_velocity"] = rs_velocity - shear_val * 0.08
    boosted_shear = shear_val * (1.0 + (float(motion_multiplier) * MOTION_RS_GAIN))
    transform = np.float32([[1, boosted_shear, -boosted_shear * h / 2], [0, 1, 0]])
    return cv2.warpAffine(image, transform, (w, h), borderMode=cv2.BORDER_REFLECT101)
...
def process_frame(
    ...
    # V17: Dynamic Stress (Gentle) = V15 (Temporal Mute) + GENTLE motion-
    # coupled optics. V14 math/encoding + V13 noise-free philosophy. No sensor
    # floor, no face tracking, no AE hunting, no rPPG. Temporal blending is
    # applied in naturalize_video. The ONLY change vs V15 is that the two
    # physical motion passes scale with motion_multiplier via the much
    # gentler MOTION_*_GAIN constants (0.0 == V15 behaviour).

    # 1. Physical motion (gently motion-coupled in V17).
    image = apply_soft_ois_jitter(image, state, rng, motion_multiplier)
    image = apply_soft_rolling_shutter(image, state, rng, motion_multiplier)
