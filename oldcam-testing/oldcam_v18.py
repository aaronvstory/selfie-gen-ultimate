#!/usr/bin/env python3
"""
oldcam_v18.py - V18 "Dynamic Blur" Virtual Hardware Simulator (STANDALONE TEST)
...
main test suite. It exists only to A/B a V18 idea against V15 through the
resemble-score deepfake API before deciding whether to promote it. Run it
directly:  python oldcam-testing/oldcam_v18.py <video> [--ghosting 0.18 ...]

V18 = clean V15 + a motion-gated GAUSSIAN BLUR. NO geometric coupling.

V16 (OIS/RS ×8/×5) and V17 (×2/×1.5) proved that adding geometric
distortion during fast motion is a dead end: the warp/shear introduced its
OWN detectable edges and *raised* the head-turn peak (0.42 → 0.54/0.56)
even when gentle. V18 abandons geometric coupling entirely. The AI still
fails structurally during fast head turns — if we can't hide that with
warp, hide it with blur. A real lens genuinely loses focus during rapid
perspective shifts, and a Gaussian blur degrades detail *uniformly*
without adding the tearing/shear artifacts the detector locked onto.

The OIS jitter and rolling shutter run at their BASELINE V15 strength
(no motion_multiplier). The only new behaviour is: when frame-to-frame
motion is high, apply a Gaussian blur whose kernel scales with motion.
At motion_multiplier ~0 (subject still) V18 == V15 exactly.
...
Inherited from V15 verbatim:
  ...
  - V13's noise-free philosophy: NO sensor noise of any kind.
    `apply_daylight_sensor_floor` / `rng.normal` synthetic noise stay purged.
...
V18 additions:
  ...
  - Crude per-frame motion velocity (grayscale absdiff -> threshold -> sum,
    normalized to a 0.0-1.0 motion_multiplier on resolution area) — the
    same calculation V16/V17 used, reused verbatim.
  - apply_motion_coupled_blur(): a Gaussian blur whose (odd) kernel scales
    motion_multiplier -> [3 .. MOTION_BLUR_MAX_KERNEL]; skipped on
    near-still frames. Placed EARLY in process_frame (right after the
    geometric passes, before bloom/CA/AWB) — a real out-of-focus capture
    loses detail *before* the ISP adds bloom/colour, not after.
...
# V18 motion-gated blur: the ONLY knob that differs from V15. The Gaussian
# kernel scales motion_multiplier (0..1) -> [3 .. this]. 15 is deliberately
# strong: motion_multiplier rarely saturates (a head turn is ~0.4-0.7), so a
# small max would make "fast" frames a no-op blur and the test inconclusive.
# Tune down if it over-softens; this is a one-line change + new oldcam_vN.py.
MOTION_BLUR_MAX_KERNEL = 15  # odd; effective kernel is clamped to <= this
...
def build_default_output_path(input_path):
    # -oldcam-v18 suffix so resemble-score classifies it as "Oldcam v18"
    # (it keys on the *-oldcam-vN.<ext> filename pattern).
    path = Path(input_path)
    return str(path.with_name(f"{path.stem}-oldcam-v18{path.suffix}"))


def build_preview_output_path(input_path):
    path = Path(input_path)
    return str(path.with_name(f"{path.stem}-preview-v18{path.suffix}"))
...
def apply_motion_coupled_blur(
    image: np.ndarray, motion_multiplier: float
) -> np.ndarray:
    """V18: Gaussian blur whose kernel scales with frame-to-frame velocity.

    Models a real lens losing focus during a rapid perspective shift. Unlike
    V16/V17's geometric coupling, blur degrades detail *uniformly* — it does
    not add the tearing/shear edges the detector locked onto. Near-still
    frames are returned untouched so V18 == V15 when the subject isn't
    moving (the only variable under test is the head-turn blur).
    """
    if motion_multiplier <= 0.05:
        return image
    # Map motion 0..1 -> kernel 0..MAX, force odd and >= 3, clamp to MAX.
    k = int(round(motion_multiplier * MOTION_BLUR_MAX_KERNEL))
    if k % 2 == 0:
        k += 1
    if k < 3:
        return image
    k = min(k, MOTION_BLUR_MAX_KERNEL if MOTION_BLUR_MAX_KERNEL % 2
            else MOTION_BLUR_MAX_KERNEL - 1)
...
def process_frame(
    ...
    # V18: Dynamic Blur Profile = clean V15 + a motion-gated Gaussian blur.
    # V14 math/encoding + V13 noise-free philosophy. No sensor floor, no face
    # tracking, no AE hunting, no rPPG. Temporal blending is applied in
    # naturalize_video. The ONLY change vs V15 is the motion-gated blur
    # inserted after the (baseline, un-coupled) geometric passes.

    # 1. Physical motion — BASELINE V15 (no motion coupling; V16/V17 proved
    #    geometric coupling adds detectable warp and hurts the score).
    image = apply_soft_ois_jitter(image, state, rng)
    image = apply_soft_rolling_shutter(image, state, rng)

    # 1b. V18 NEW: motion-gated Gaussian blur, placed EARLY — a real lens
    #     loses focus during a fast perspective shift *before* the ISP adds
    #     bloom/colour. Blurs the AI's structural-failure frames during the
    #     head turn without introducing geometric tearing. ~No-op when still.
    image = apply_motion_coupled_blur(image, motion_multiplier)
...
def naturalize_video(input_path: str, output_path: str, args: argparse.Namespace) -> None:
    ...
    previous_frame = None  # V16: raw (pre-process) prev frame for motion delta
    ...
    try:
        while True:
            ...
            # V16: crude, fast frame-to-frame motion velocity. Grayscale
            # absdiff -> threshold (kills sensor micro-noise) -> count moved
            # pixels, normalized to a 0.0-1.0 multiplier on resolution area.
            # The 0.15 divisor means ~15% of the frame changing saturates the
            # multiplier (a fast head turn), so the optical-stress boost ramps
            # smoothly and is ~0 when the subject is still (==V15 behaviour).
            if previous_frame is not None:
                ...
                motion_multiplier = float(
                    np.clip(
                        (np.sum(thresh) / 255.0) / (total_pixels * 0.15),
                        0.0,
                        1.0,
                    )
                )
            ...
            current_processed = process_frame(
                frame, None, vignette_mask, args, rng, state, motion_multiplier
            )
