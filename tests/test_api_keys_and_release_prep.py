...
def test_bundle_release_creates_universal_zip_with_top_level_launchers(tmp_path: Path):
    ...
    with zipfile.ZipFile(versioned_zip) as zf:
        ...
        assert any(name.endswith("selfie-gen-ultimate/launchers/run_similarity_gui.bat") for name in names)
        assert any(name.endswith("selfie-gen-ultimate/launchers/run_similarity_cli.bat") for name in names)
        assert any(name.endswith("selfie-gen-ultimate/launchers/run_oldcam_v8.bat") for name in names)
        assert any(name.endswith("selfie-gen-ultimate/launchers/run_oldcam_v7.bat") for name in names)
        assert any(name.endswith("selfie-gen-ultimate/launchers/run_similarity_gui.command") for name in names)
        assert any(name.endswith("selfie-gen-ultimate/launchers/run_similarity_cli.command") for name in names)
        assert any(name.endswith("selfie-gen-ultimate/launchers/run_oldcam_v8.command") for name in names)
        assert any(name.endswith("selfie-gen-ultimate/launchers/run_oldcam_v7.command") for name in names)
