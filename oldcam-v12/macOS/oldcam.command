...
find_repo_root() {
  ...
  while [ -n "$cur" ] && [ "$cur" != "/" ]; do
    if [ -f "$cur/kling_automation_ui.py" ] && [ -f "$cur/requirements.txt" ] && [ -d "$cur/oldcam-v12" ]; then
      printf '%s\n' "$cur"
      return 0
    fi
    cur="$(dirname "$cur")"
...
set selectedFiles to choose file with prompt "Select one or more media files for Oldcam V12" with multiple selections allowed
set outputText to ""
